...
# Assemble Training and Testing Index Lists
random.seed(1234)
train_percentage = .8
training_indexes = random.sample( range(len(full_dataset)) , int(len(full_dataset)*train_percentage) )
testing_indexes = [x for x in range(len(full_dataset)) if x not in training_indexes]

assert(set(training_indexes).isdisjoint(testing_indexes))
assert((len(testing_indexes)+len(training_indexes)) == len(full_dataset))

# Create dataloaders for training and test data
trainset = torch.utils.data.Subset(full_dataset, training_indexes)
testset = torch.utils.data.Subset(full_dataset, testing_indexes)
loaders = {'train': torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=16),
           'val': torch.utils.data.DataLoader(testset, batch_size=batch_size, shuffle=True, num_workers=16)}
...
#exp_lr_scheduler = lr_scheduler.StepLR(optimizer_ft, step_size=num_epochs+1, gamma=0.1) # Do not use for now
